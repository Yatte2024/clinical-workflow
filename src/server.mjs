...
const prototypeDir = path.join(root, "prototypes");
const dataDir = path.join(root, "data");
const inputFiles = {
  adam: path.join(dataDir, "adam-specs.yaml"),
  dpp: path.join(dataDir, "output-dpp.yaml"),
};
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/") {
      return send(res, 200, await readFile(path.join(publicDir, "index.html"), "utf8"), "text/html; charset=utf-8");
    }
    if (req.method === "GET" && url.pathname === "/dag-test.html") {
      const html = await readFile(path.join(prototypeDir, "dag-test.html"), "utf8");
    ...
    if (req.method === "GET" && url.pathname === "/focus-dag-test.html") {
      const html = await readFile(path.join(prototypeDir, "focus-dag-test.html"), "utf8");
      const payload = JSON.stringify(JSON.stringify(await generateArtifacts()).replaceAll("<", "\\u003c"));
      return send(res, 200, html.replace('"__FOCUS_DAG_STATE_JSON__"', payload), "text/html; charset=utf-8");
    }
    if (req.method === "GET" && url.pathname === "/adrg-test.html") {
      return send(res, 200, await readFile(path.join(prototypeDir, "adrg-test.html"), "utf8"), "text/html; charset=utf-8");
    }
